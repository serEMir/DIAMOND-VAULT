...
var addressSchema = exports_external.string().regex(/^0x[a-fA-F0-9]{40}$/, "Expected a 20-byte EVM address");
var bytes32Schema = exports_external.string().regex(/^0x[a-fA-F0-9]{64}$/, "Expected a 32-byte hex string");
var HarvestConfigSchema = exports_external.object({
...
init_toHex();
init_getAddress();
var STRATEGY_HARVESTED_EVENT_ABI = [
  {
    type: "event",
...
function encodeKeeperHarvestCall(strategyId) {
...
function harvestCallback(runtime2) {
  const config = runtime2.config;
  runtime2.log(`harvest triggered - strategyId: ${config.strategyId}, chain: ${config.chainSelectorName}`);
...
function initWorkflow(config) {
  ...
  return [
    cre.handler(cron.trigger({ schedule: config.cronSchedule }), (runtime2) => {
      return harvestCallback(runtime2);
    })
  ];
}
async function main() {
